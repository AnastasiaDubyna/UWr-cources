...
(define (parse-expr e)
  (cond
    ...
    [(and (list? e)
          (name-fun? (first e))
          (> (length e) 0))
     (call (car e) (map parse-expr (cdr e)))] ;; dodane w zadaniu 1
    [else (error e 'parse-error)]))
...
(define (parse-fun e)
  (cond
    [(and (= (length e) 4)
          ...
          (name-fun? (second e))
          (list? (third e))
          (andmap name-var? (third e)))
     (fun (second e)
          (third e)
          (parse-expr (fourth e)))] ;; dodane w zadaniu 1
...
(define (parse-prog e)
  (cond
    [(and (cons? e)
          (list? (first e))
          (andmap list? (first e)))
     (prog (map parse-fun (first e)) (parse-expr (second e)))] ;; dodane w zadaniu 1
    [else (error 'parse-error)]))
  ...
  (match e
    ...
    [(var-expr x)
     (match (env-lookup x ρ)
       ...
       [v v])]
    [(let-expr x e1 e2)
     (let* ((v (eval-expr θ ρ e1))
            (ρ (env-add x v ρ)))
       (eval-expr θ ρ e2))]
    [(if-expr eb et ef)
     (match (eval-expr θ ρ eb)
       [#t (eval-expr θ ρ et)]
       [#f (eval-expr θ ρ ef)]
       [v  (raise (type-error 'boolean v))])]
    [(binop op el er)
     (let* ((vl (eval-expr θ ρ el))
            (vr (eval-expr θ ρ er)))
       ((op->proc op) vl vr))]
    [(call f es) ; dodane w zadaniu 2
     (let ([fn (env-lookup f θ)])
       (if (not-found? fn)
           (error 'undefined-function)
           (if (= (length es) (length (fun-params fn)))
               (let* ([vs (map (λ (e) (eval-expr θ ρ e)) es)]
                      [ρ2 (foldr (λ (x v ρ-tmp) (env-add x v ρ-tmp))
                                 ρ
                                 (fun-params fn)
                                 vs)])
                 (eval-expr θ ρ2 (fun-body fn)))
               (error 'arity-mismatch))))]))

(define (eval-prog p) ; dodane w zadaniu 2
  (define θ (foldr (λ (fn env) (env-add (fun-name fn) fn env))
                   env-empty
                   (prog-funs p)))
  (eval-expr θ env-empty (prog-expr p)))
