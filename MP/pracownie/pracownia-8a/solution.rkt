...
(struct unbound-var (id) #:transparent) ; DODANE
...
(define (apply-fn fn ls) ; DODANE
  (if (null? ls) fn
      (match fn
    [(clo id body env)
                  (apply-fn (eval-env body (env-add env id (car ls))) (cdr ls))]
    [_            (raise type-error 'procedure fn)])))
...
(define (make-list es) ;; DODANE
  (if (null? es)
      (const null)
      (pair-expr (car es) (make-list (cdr es)))))
