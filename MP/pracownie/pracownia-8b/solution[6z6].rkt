...
(define (eval-env e ρ)
  (match e
    [(const n)    n]
    [(var-expr x)
     (let [(xval (env-lookup ρ x))]
       (match xval
         [(lclo body env) (eval-env body env)]
         [else xval]))]
    [(fun x e)    (clo x e ρ)]
    [(app e1 e2)
    ...
    [(pair-expr el er)
     (cons (lclo el ρ) (lclo er ρ))]
    [(fst e)
     (let ((v (eval-env e ρ)))
       (if (pair? v)
           (match (car v)
             [(lclo body env) (eval-env body env)]
             [else (raise (type-error 'pair (car v)))])
    ...
    [(snd e)
     (let ((v (eval-env e ρ)))
       (if (pair? v)
           (match (cdr v)
             [(lclo body env) (eval-env body env)]
             [else (raise (type-error 'pair (cdr v)))])
           (raise (type-error 'pair v))))]
    [(let-expr x e1 e2)
     (let ((v (eval-env e1 ρ)))
